...
int main(int argc, char** argv) {
	...
	if(proc_size > height) proc_size = height;		//One worker get at least one row
	if(rank >= proc_size) {
	...
	slave_batch = (int)sqrt(width * height);
	...
	if(proc_size > 1) {
		if(rank > 0) {
			...
			assign_index[0] = (rank-1)*slave_batch;
			assign_index[1] = rank*slave_batch;
			...
			while(1){
				...
				for(int i = assign_index[0]; i < assign_index[1]; i++) {
				...
				MPI_Isend(slave_buff, 2+slave_batch, MPI_INT, 0, 0, VALID_COMM, &send_req);
				//if(assign_index[1] == width*height) break;
				MPI_Recv(assign_index, 2, MPI_INT, 0, 0, VALID_COMM, &st);
			}
		}
		else if(rank == 0) {
			int current_index, total = 0, end = -1;
			...
			while(1) {
                ...
                if(flag != 0) {
					...
					if(current_index+slave_batch >= width*height) {
						...
						send_index[0] = current_index;
						send_index[1] = current_index + slave_batch;
						current_index += slave_batch;
					}
					...
					double y0 = (current_index/width)*vert_int + lower;
					double x0 = (current_index%width)*hori_int + left;	
					int repeats = 0;
                	double x = 0;
                	double y = 0;
                	double length_squared = 0;
                	...
                	image_loc[current_index++] = repeats;
					total++;	
					//printf("Master - current_index: %d\n", current_index);
					if(current_index >= width * height) break;
				}
